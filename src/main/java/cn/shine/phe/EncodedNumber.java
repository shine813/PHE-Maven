...
import cn.shine.phe.Util.Frexp;
import cn.shine.phe.Util.OverflowErrorException;
...
public class EncodedNumber {
     ...
     */
    public EncodedNumber(PaillierPublicKey publicKey, BigInteger encoding, BigInteger exponent) {
     ...
     * @throws ValueErrorException if some value is lager than publicKey.maxInt
     * @throws TypeErrorException  if class of scalar is not BigInteger or BigDecimal
     * @see EncodedNumber#encode(PaillierPublicKey, Object, Double, Integer)
     */
    public static @NotNull EncodedNumber encode(@NotNull PaillierPublicKey publicKey, Object scalar) throws ValueErrorException, TypeErrorException {
     ...
     * @see EncodedNumber#encode(PaillierPublicKey, Object, Double, Integer)
     */
    public static @NotNull EncodedNumber encode(@NotNull PaillierPublicKey publicKey, Object scalar, Double precision) throws ValueErrorException, TypeErrorException {
     ...
     * @throws TypeErrorException  if class of scalar is not BigInteger or BigDecimal
     */
    public static @NotNull EncodedNumber encode(@NotNull PaillierPublicKey publicKey, Object scalar, Double precision, Integer maxExponent) throws ValueErrorException, TypeErrorException {
        ...
        if (precision == null) {
            if (scalar instanceof BigInteger) {
                ...
                // get scalar exp
                Integer binFltExponent = Frexp.frexp(scalarBigDecimal.doubleValue()).exponent;
                ...
                throw new TypeErrorException(String.format("Don't know the precision of type %s", scalar.getClass()));
            }
        } else {
        ...
        if (intRep.abs().compareTo(publicKey.maxInt) > 0) {
            ...
            throw new ValueErrorException(String.format(exception, publicKey.maxInt, intRep.abs()));
        }
     ...
     * @throws ValueErrorException if this#encoding is larger than publicKey#n
     * @throws StackOverflowError  if decode numeric overflow
     */
    public Object decode() throws ValueErrorException, StackOverflowError {
        BigInteger mantissa;

        if (this.encoding.compareTo(this.publicKey.n) >= 0) {
            throw new ValueErrorException("Attempted to decode corrupted number");
            ...
            throw new OverflowErrorException("Overflow detected in decrypted number");
        }
        ...
        if (this.exponent.compareTo(BigInteger.ZERO) >= 0) {
            ...
            try {
                ...
                throw new OverflowErrorException(String.format("decoded result too large for a float: from %s", e));
            }
        }
    }
     ...
     */
    public EncodedNumber decreaseExponentTo(@NotNull BigInteger newExp) throws ValueErrorException {
